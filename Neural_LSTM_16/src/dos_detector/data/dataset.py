...
def _build_sequence_samples(
    ...
    features = frame[feature_columns].to_numpy(dtype=DEFAULT_NUMPY_DTYPE)
    binary_labels = frame["attack"].to_numpy(dtype=DEFAULT_NUMPY_DTYPE)
...
def collate_fn(batch: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    ...
    def _to_tensor(value, dtype):
        if dtype in (torch.float16, torch.bfloat16):
            return safe_cast_tensor(value, dtype)

...
This directory hosts the MoE gate that fuses the existing detectors in the SDN/IoT stack using the shared autoencoder plus the CNN experts (the LSTM specialists are no longer part of the gateway path).

- **Autoencoder**: `autoencoder/data/artifacts/model.pt`
- **DoS branch**: CNN `dosdet/artifacts/model_best.pt`
- **ARP branch**: CNN `arpdet/artifacts/model_best.pt`
- **Shared autoencoder**: `autoencoder/data/artifacts/model.pt` supplies window-level context for both gates

The gating network dynamically selects the contribution of each expert model depending on input traffic characteristics, enabling context-aware attack discrimination between volumetric (DoS) and spoofing (ARP) threats within a unified pipeline using only the autoencoder and the CNN branches.
...
- Legacy checkpoints from the older five-expert layout are loaded by trimming the LSTM rows/columns; the gateway now routes attention across the autoencoder plus the DoS and ARP CNN experts.
...
If you load older checkpoints saved when the gateway contained five experts (autoencoder, DoS CNN, DoS LSTM, ARP CNN, ARP LSTM), the loader trims weights to keep the autoencoder plus the DoS and ARP CNN experts. Frozen LSTM weights are ignored because the gateway no longer depends on them.
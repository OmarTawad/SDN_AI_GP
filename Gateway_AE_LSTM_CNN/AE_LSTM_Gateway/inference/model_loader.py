...
def _remap_expert_axes(state_dict: Mapping[str, torch.Tensor], keep_indices, target_num_experts: int):
    """Drop CNN expert rows/cols when loading legacy checkpoints."""
...
def _align_state_dict(model: torch.nn.Module, state_dict: Mapping[str, torch.Tensor]) -> Mapping[str, torch.Tensor]:
    """Trim or filter checkpoint weights to match the LSTM-only architecture."""

    target_state = model.state_dict()
    keep_indices = (0, 2, 4)  # auto, DoS LSTM, ARP LSTM in the legacy 5-expert ordering

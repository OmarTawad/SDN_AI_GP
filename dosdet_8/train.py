...
class CachedDataset(Dataset):
    ...
    def __init__(
        ...
        if take_indices is not None:
            take_indices = [int(i) for i in take_indices if 0 <= int(i) < len(self.df)]
            if not take_indices:
                raise IndexError("All requested take_indices were out of bounds for the dataset.")
            self.df = self.df.iloc[take_indices]
    ...
    for epoch in range(1, tr_cfg["epochs"] + 1):
        ...
        with torch.no_grad():
            for batch in tqdm(val_thin_loader, desc="Thin-val", unit="batch", leave=False):
                ...
                stat_scaled = scaler.transform(stat_np, feature_names)
                stat_slim = slimmer.transform(stat_scaled)
                static_t = torch.from_numpy(stat_slim).to(device, dtype=torch.float32)

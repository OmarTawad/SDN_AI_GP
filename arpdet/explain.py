...
def save_attention_heatmaps(
    ...
    with torch.no_grad():
        for idx in range(len(ds)):
            ...
            attn = out.get("attn")
            attn_vec = attn.cpu().numpy().ravel() if attn is not None else np.zeros(seq.shape[0], dtype=np.float32)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Permutation importance + attention visualisations for ARP spoofing CNN")

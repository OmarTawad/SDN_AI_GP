...
from ..data.dataset import SequenceDataset, collate_fn, filter_normal_sequences
...
# Speed knobs (safe no-ops on CPU)
...
class AutoencoderTrainer:
    ...
    def train(self) -> Dict[str, float]:
        ...
        train_loader = DataLoader(train_dataset, batch_size=self.config.training.autoencoder.batch_size, shuffle=True, num_workers=self.config.training.autoencoder.num_workers, collate_fn=collate_fn, pin_memory=True, prefetch_factor=4, persistent_workers=True)
        val_loader = DataLoader(val_dataset, batch_size=self.config.training.autoencoder.batch_size, shuffle=False, num_workers=self.config.training.autoencoder.num_workers, collate_fn=collate_fn, pin_memory=True, prefetch_factor=4, persistent_workers=True)
        ...
        for epoch in progress(range(1, self.config.training.autoencoder.max_epochs + 1), desc="Autoencoder epochs", unit="ep"):
            ...
            self.logger.info("ae_epoch_end", epoch=epoch, train_loss=train_loss, val_loss=val_loss)
    ...
    def _train_epoch(
        ...
        for step, batch in enumerate(progress(loader, desc="Batches (AE)", unit="batch", leave=False)):

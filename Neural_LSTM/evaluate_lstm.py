...
from dos_detector.utils import CPU_DEVICE, configure_cpu_environment, resolve_project_root
...
def _evaluate(model: SequenceClassifier, loader: DataLoader, device: torch.device, threshold: float) -> tuple[dict, np.ndarray, str]:
...
def main() -> None:
    ...
    loader = _build_loader(files, config, feature_columns, args.batch_size, scaler)
    ...
    start = time.time()
    metrics, cm, report = _evaluate(model, loader, device, args.threshold)

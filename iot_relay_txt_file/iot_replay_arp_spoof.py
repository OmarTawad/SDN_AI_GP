...
class IoTTrafficReplayer(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Real device MAC -> Device name map (lowercase keys)
        self.mac_to_device = {
        ...
        self.datapath = None
        self.packet_interval = 1  # seconds between packets
        self.attack_interval = 10  # send ARP spoof every 10 packets
        self.csv_file = "iot_traffic.csv"
        self.logger.info("Starting IoT replay + ARP spoofing simulation.")
    ...
    def _replay_traffic_loop(self):
        time.sleep(5)  # Wait for switch connection
        packet_counter = 0
            ...
            try:
                with open(self.csv_file, "r") as f:
                    ...
                    for row in reader:
                        ...
                        # Send ARP spoof packets every attack_interval packets
    ...
    def _send_arp_spoof(self):
        ...
        # Pick two distinct random devices to spoof between
        devices = list(self.mac_to_device.keys())
        malicious_mac = "00:00:00:00:00:14"  # Malicious host MAC (must be in map)
        victim_mac = None
        victim_ip = None
        spoofed_ip = None

        # Pick victim and spoofed IP from known devices (excluding malicious)
        ...
        # For IPs, since CSV has IPs, ideally parse or hardcode some IPs mapped to MACs here:
        # For demo, assume IP is 192.168.1.<last_octet> with last octet from MAC's last byte
        def mac_to_ip(mac):
        ...
        p.add_protocol(arp.arp(
            ...
            src_ip=spoofed_ip,  # claiming to be spoofed IP

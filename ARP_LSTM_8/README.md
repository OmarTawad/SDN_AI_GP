...
This `ARP_LSTM_8` variant keeps the exact same LSTM architecture but enforces CUDA-only int8 fake-quantization. Artifacts are written to `models_int8/` and reports to `reports_int8/` so the FP32/FP16 workflows remain untouched.
...
CUDA 11.x GPU is required for the int8 pipeline (no CPU fallback).

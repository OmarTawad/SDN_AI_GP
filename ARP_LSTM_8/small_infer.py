...
# Small-window inference helper for the ARP_LSTM detector (INT8).
...
def _infer_window(
    ...
    tensor = torch.from_numpy(scaled).to(device=device, dtype=torch.float32).unsqueeze(0)
...
def infer_windows(
    ...
    for idx, window in enumerate(windows):
        ...
        results.append(_infer_window(window, row, feature_columns, model, scaler, device, tau))
    return results


def main() -> None:
    parser = argparse.ArgumentParser(description="Small-window inference (ARP_LSTM INT8).")
    ...
    windows_out = infer_windows(windows, frame, feature_columns, model, scaler, device, tau)

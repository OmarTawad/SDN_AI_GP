...
def _decode_packet(data: bytes, timestamp: float) -> Optional[PacketRecord]:
    ...
    if ip_layer is not None:
        ...
        arp_opcode = int(getattr(arp, "op", 0)) or None
        arp_sender_ip = getattr(arp, "psrc", None)
        arp_sender_mac = getattr(arp, "hwsrc", None)
        arp_target_ip = getattr(arp, "pdst", None)
        arp_target_mac = getattr(arp, "hwdst", None)
        src_ip = arp_sender_ip or src_ip
        dst_ip = arp_target_ip or dst_ip
        payload_len = len(bytes(arp.payload))
        arp_is_gratuitous = bool(
            (arp_opcode == 2 and arp_sender_ip and arp_target_ip and arp_sender_ip == arp_target_ip)
            or (arp_opcode == 2 and arp_sender_mac and arp_target_mac and arp_sender_mac == arp_target_mac)
        )
    else:
...
def read_pcap(path: Path, limit: Optional[int] = None, byte_limit: Optional[int] = None) -> List[PacketRecord]:
    ...
    try:
        ...
        for index, (data, meta) in it:
            ...
            if index == 0:
                raise RuntimeError(f"DEBUG_CHECK_TS: {ts} (Raw from pcap_reader)")

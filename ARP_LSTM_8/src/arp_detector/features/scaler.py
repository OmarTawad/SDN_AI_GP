...
class RobustScaler:
    ...
    def load(cls, dirpath: str) -> "RobustScaler":
        ...
        """
        # Support loading from file path or dir
        if os.path.isfile(dirpath):
            ...
            if not os.path.exists(path):
                 # Try variations
                 candidates = [
                     os.path.join(dirpath, "feature_scaler_int8.joblib"),
                     os.path.join(dirpath, "scaler.joblib")
                 ]
                 for c in candidates:
                     if os.path.exists(c):
                         import joblib
                         return joblib.load(c)
                 raise FileNotFoundError(f"No scaler found in {dirpath}")

...
class InferencePipeline:
    ...
    def _load_supervised_model(self) -> SequenceClassifier:
        ...
        state = torch.load(self.config.paths.supervised_model_path, map_location="cpu")
    ...
    def _run_supervised(self, raw_frame: pd.DataFrame, loader: DataLoader) -> Dict[int, Dict[str, object]]:
        ...
        with torch.no_grad():
            for batch in progress(loader, desc="Inference", unit="batch", leave=False):
                features = batch["features"].to(self.device, non_blocking=self.use_cuda)
    ...
    def _explain_sequence(self, dataset: SequenceDataset, window_index: int) -> Dict[str, object]:
        ...
        # cudnn RNN backward requires training mode; restore prior state afterward
        was_training = self.supervised_model.training
        ...
        for sample in dataset.samples:
            ...
            if int(start) <= window_index <= int(end):
                ...
                result = {"top_features": top_features, "temporal_attention": attention}
                if not was_training:

...
Modernised training/inference stack for the simulated SDN IoT ARP-spoofing detection project. The repository focuses on a single workflow: preprocess raw pcaps into parquet shards, train the CNN detector, and run calibrated inference with conflict-aware post-processing. This `arpdet_16` variant keeps the same flow but defaults to AMP/FP16 on CUDA and writes outputs to separate `artifacts_fp16/` + `reports_fp16/` directories so FP32 runs remain untouched.

#!/usr/bin/env python3
"""Evaluate the CNN-based ARP spoofing detector and persist metrics."""
...
def _test_paths(cfg: dict) -> List[str]:
    ...
    for rel in shard_rel:
        ...
        y = _read_parquet(str(path), columns=["y"])["y"]
        stats.append({"path": str(path), "pos": int(y.sum())})
    ...
    if buckets[1] and sum(s["pos"] for s in buckets[1]) == 0:
        ...
        if donor:
            src = buckets[0] if donor in buckets[0] else buckets[2]
            src[src.index(donor)] = buckets[1][0]
            buckets[1][0] = donor
...
def _build_loader(cfg: dict, batch_size: int) -> DataLoader:
    ...
    workers = max(0, cfg["training"].get("dataloader_workers", 0))
    return DataLoader(dataset, batch_size=batch_size, shuffle=False, collate_fn=collate, num_workers=workers)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate the ARP CNN on cached test data.")
    ...
    print("\u2705 \u201carpdet_8 evaluation complete \u2013 metrics saved under arpdet_8/eval/metrics.json\u201d")

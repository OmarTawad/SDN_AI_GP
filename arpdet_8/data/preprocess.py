...
def preprocess(cfg: dict, pcaps_glob: str | List[str], labels_csv: str):
    ...
    def _open_new_shard():
        nonlocal shard_id, shard_path, shard_writer, bytes_written_in_shard
    ...
    def _flush_batch():
        ...
        # Encode variable-length arrays as JSON strings for robust parquet writing
        ...
        if use_csv:
            ...
            shard_writer = True  # sentinel so append_mode works on subsequent calls
            est = int(df.memory_usage(deep=True).sum())
...
def main():
    ...
    ap.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional glob override (supports multiple patterns); defaults to config preprocess.pcaps_glob",
    )

...
def _resolve_fp16_infer_config(config: Config) -> tuple[torch.device, torch.dtype, bool, float]:
    ...
    if not use_fp16:
        raise RuntimeError("train.fp16_cpu must be true to enforce CPU FP16 inference.")
...
def load_inference_artifacts(config: Config) -> InferenceArtifacts:
    ...
    device, torch_dtype, fp16_clamp, fp16_max = _resolve_fp16_infer_config(config)

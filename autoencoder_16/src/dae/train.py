...
def _resolve_fp16_training_config(train_cfg: dict) -> Tuple[torch.device, torch.dtype, bool, float]:
    ...
    if not use_fp16:
        raise RuntimeError("train.fp16_cpu must be true to enforce CPU FP16 training.")
...
def train_epoch(
    ...
    total = 0
    for (batch,) in loader:
        inputs = batch.to(device=device, dtype=torch_dtype)
        if fp16_clamp:
            inputs = torch.clamp(inputs, min=-fp16_max, max=fp16_max)
        if denoising and noise_std > 0:
            noise = torch.randn_like(inputs) * noise_std
            inputs_noisy = inputs + noise
            if fp16_clamp:
                inputs_noisy = torch.clamp(inputs_noisy, min=-fp16_max, max=fp16_max)
        else:
            inputs_noisy = inputs
...
def train_model(config: Config, windows_path: Path) -> TrainingArtifacts:
    ...
    device, torch_dtype, fp16_clamp, fp16_max = _resolve_fp16_training_config(train_cfg)
    train_scaled = train_scaled.astype(np.float16, copy=False)
    val_scaled = val_scaled.astype(np.float16, copy=False)
    ...
    for start in range(0, total_rows, eval_batch_size):
        ...
        scaled_chunk = scaler.transform(feature_chunk.values).astype(np.float16, copy=False)
        tensor = torch.from_numpy(scaled_chunk).to(device=device, dtype=torch_dtype)
        if fp16_clamp:

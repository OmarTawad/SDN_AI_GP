...
def _resolve_fp16(config: Config, device_str: str) -> tuple[torch.device, torch.dtype, bool, float]:
    device_value = str(device_str or "cpu").lower()
    if device_value != "cpu":
        raise RuntimeError("autoencoder_16 enforces CPU-only fp16 evaluation; use --device cpu")
    train_cfg = config.get("train") or {}
    cfg_device = str(train_cfg.get("device", "cpu")).lower()
    if cfg_device != "cpu":
        raise RuntimeError("autoencoder_16 enforces CPU-only fp16; set train.device: cpu")
    use_fp16 = bool(train_cfg.get("fp16_cpu", True))
    if not use_fp16:
        raise RuntimeError("train.fp16_cpu must be true to enforce CPU FP16 evaluation.")
    fp16_clamp = bool(train_cfg.get("fp16_clamp", True)) and use_fp16
    fp16_max = float(torch.finfo(torch.float16).max)
    return torch.device("cpu"), torch.float16, fp16_clamp, fp16_max
...
def _score_batches(
    ...
    with torch.no_grad():
        for start in range(0, len(array), batch_size):
            tensor = torch.from_numpy(array[start : start + batch_size]).to(device=device, dtype=torch_dtype)
            if fp16_clamp:
                tensor = torch.clamp(tensor, min=-fp16_max, max=fp16_max)
            recon = model(tensor)
...
def _load_artifacts(config: Config, override: str | None, device_str: str):
    device, torch_dtype, fp16_clamp, fp16_max = _resolve_fp16(config, device_str)
    art_dir = _artifact_dir(config, override)
...
def _collect_windows(arg_path: str | None, config: Config) -> List[Path]:
    ...
    return sorted(dict.fromkeys(candidates), key=lambda p: (next((i for i, tok in enumerate(SPLIT_TOKENS) if tok in p.stem.lower()), len(SPLIT_TOKENS)), p.name))
...
def _evaluate_dataset(paths: Sequence[Path], artifacts, label_hint: str | None, rows_per_batch: int, batch_size: int, threshold_override: float | None):
    ...
    for path in paths:
        for chunk in _iter_windows(path, rows_per_batch):
            ...
            errors = _score_batches(
                artifacts["model"],
                scaled,
                artifacts["device"],
                batch_size,
                artifacts["dtype"],
                artifacts["fp16_clamp"],
                artifacts["fp16_max"],
            )

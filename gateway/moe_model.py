...
def _infer_lstm_input_dim(weight_path: Path, fallback_dim: int) -> int:
    """Infer LSTM input width from a checkpoint's ``rnn.weight_ih_l0`` tensor."""
    ...
    for key in ("rnn.weight_ih_l0", "module.rnn.weight_ih_l0"):
        tensor = state_dict.get(key)
        if isinstance(tensor, Tensor) and tensor.dim() == 2:
            return int(tensor.shape[1])
    return fallback_dim


DOS_LSTM_INPUT_DIM = _infer_lstm_input_dim(NEURAL_LSTM_ROOT / "models" / "supervised.pt", DOS_LSTM_INPUT_DIM)
ARP_LSTM_INPUT_DIM = _infer_lstm_input_dim(ARP_LSTM_ROOT / "models" / "supervised.pt", ARP_LSTM_INPUT_DIM)
...
DOS_TASK_SPEC = TaskMoESpec(
    ...
    expert_specs=(AUTOENCODER_SPEC, DOS_LSTM_SPEC),
)

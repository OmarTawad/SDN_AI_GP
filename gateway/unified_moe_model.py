"""Unified Mixture-of-Experts architecture for three-class traffic classification."""
...
class UnifiedGating(nn.Module):
    """Two-layer gating network that produces a distribution over frozen experts."""
...
class UnifiedMoE(nn.Module):
    ...
    def train(self, mode: bool = True) -> UnifiedMoE:  # type: ignore[override]
        super().train(mode)
        # Experts remain frozen in eval mode regardless of training toggles.
        for expert in self.experts:
            expert.eval()
        return self

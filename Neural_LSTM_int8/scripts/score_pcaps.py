...
from dos_detector.utils.io import load_dataframe, load_json, ensure_dir
...
def per_file_scores(
    ...
    if df.empty or not feat_cols:
        return np.array([], dtype=int), np.array([], dtype=DEFAULT_NUMPY_DTYPE)

    X = df[feat_cols].to_numpy(dtype=DEFAULT_NUMPY_DTYPE)
    X = scaler.transform(X).astype(DEFAULT_NUMPY_DTYPE, copy=False)
    ...
    with torch.no_grad():
        if len(X) < seq_len:
            ...
            logits = (
                model(safe_cast_tensor(Xb, torch_dtype or DEFAULT_TORCH_DTYPE).to(device))
                .window_logits.detach()
                .cpu()
                .numpy()[0, :len(X)]
            )
            probs = 1/(1+np.exp(-logits))
            return widx, probs.astype(DEFAULT_NUMPY_DTYPE)
        ...
        probs = 1/(1+np.exp(-logits))
        return np.array(flat_idx[:len(probs)], dtype=int), probs.astype(DEFAULT_NUMPY_DTYPE)
...
def main():
    ...
    torch_dtype, _ = resolve_precision_mode(getattr(cfg.training.supervised, "precision_mode", None))
    ...
    model.load_state_dict(state); model = model.to(device=device, dtype=torch_dtype or DEFAULT_TORCH_DTYPE).eval()
    ...
    for pcap in pcaps:
        ...
        wi, pr = per_file_scores(df, feat_cols, scaler, model, seq_len, torch_dtype, device)
        if pr.size == 0:

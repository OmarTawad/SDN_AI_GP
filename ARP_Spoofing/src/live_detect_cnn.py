#!/usr/bin/env python3
"""
live_detect_cnn.py — ARP spoofing detection with PyTorch CNN + behavior score.
"""
...
class CNNClassifier(nn.Module):
    def __init__(self, input_channels: int, timesteps: int, dropout: float = 0.2):
        super().__init__()
        self.conv1 = nn.Conv1d(input_channels, 64, kernel_size=3, padding=0)
        self.act1 = nn.ReLU()
        self.drop1 = nn.Dropout(dropout)
        self.conv2 = nn.Conv1d(64, 32, kernel_size=3, padding=0)
        self.act2 = nn.ReLU()
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(32 * max(1, timesteps - 4), 64)
        self.act3 = nn.ReLU()
...
def run_detection(args):
    ...
    X_raw, feature_order = align_features_with_scaler(df, scaler)
    X_scaled = scaler.transform(X_raw)

    timesteps = int(args.seq_len or 1)
    X_for_model = build_rolling_sequences(X_scaled, timesteps)

    # load model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    input_features = X_for_model.shape[2]
    model = CNNClassifier(input_channels=input_features, timesteps=timesteps).to(device)
    model.load_state_dict(torch.load(args.model_path, map_location=device))
    model.eval()

    with torch.no_grad():
        inputs = torch.from_numpy(X_for_model).float().to(device)
        preds = model(inputs).cpu().numpy()

    # sliding window behavior score
    ...
    mac_window_ips = defaultdict(lambda: defaultdict(set))
...
# ---------------- CLI ----------------
def parse_args():
    p = argparse.ArgumentParser(description="ARP spoofing detection (PyTorch CNN)")
    p.add_argument("--csv", required=True, help="Feature CSV to analyze")
    p.add_argument("--model-path", required=True, help="PyTorch CNN model (.pt)")
    p.add_argument("--scaler-path", required=True, help="joblib scaler (.pkl)")

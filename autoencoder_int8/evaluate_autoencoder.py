...
def _resolve_int8(config: Config, device_str: str) -> tuple[torch.device, torch.dtype, dict]:
    device_value = str(device_str or "cpu").lower()
    if device_value != "cpu":
        raise RuntimeError("autoencoder_int8 enforces CPU-only evaluation; use --device cpu")
    train_cfg = config.get("train") or {}
    cfg_device = str(train_cfg.get("device", "cpu")).lower()
    if cfg_device != "cpu":
        raise RuntimeError("autoencoder_int8 enforces CPU-only evaluation; set train.device: cpu")
...
def _load_artifacts(config: Config, override: str | None, device_str: str):
    device, torch_dtype, quant_cfg = _resolve_int8(config, device_str)

...
def _resolve_int8_training_config(train_cfg: dict) -> Tuple[torch.device, torch.dtype]:
    device_value = str(train_cfg.get("device", "cpu")).lower()
    if device_value != "cpu":
        raise RuntimeError("autoencoder_int8 enforces CPU-only training; set train.device: cpu")
...
def train_model(config: Config, windows_path: Path) -> TrainingArtifacts:
    ...
    device, torch_dtype = _resolve_int8_training_config(train_cfg)
    ...
    for start in range(0, total_rows, eval_batch_size):
        ...
        scaled_chunk = scaler.transform(feature_chunk.values).astype(np.float32, copy=False)

...
def quantized_modules_present(model: nn.Module) -> bool:
    ...
    if qdynamic is not None:
        candidates.append(getattr(qdynamic, "Linear", None))
    qao = getattr(torch, "ao", None)
    qnn = getattr(qao, "nn", None) if qao else None
    qquantized = getattr(qnn, "quantized", None) if qnn else None
    qdynamic_ao = getattr(qquantized, "dynamic", None) if qquantized else None
    if qdynamic_ao is not None:
        candidates.append(getattr(qdynamic_ao, "Linear", None))
    qtypes = tuple(cls for cls in candidates if cls is not None)

...
class TemporalAttention(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            context: (batch, features), weights: (batch, time)
        """
        transformed = torch.tanh(self.query(inputs) + self.key(inputs))
        scores = self.score(transformed).squeeze(-1)      # (batch, time)
        weights = torch.softmax(scores, dim=1)            # (batch, time)
        context = torch.einsum("btd,bt->bd", inputs, weights)  # (batch, features)

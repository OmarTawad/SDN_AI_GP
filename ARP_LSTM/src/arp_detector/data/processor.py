...
class FeaturePipeline:
    def __init__(self, config: Config) -> None:
        ...
        self._last_host_maps: Dict[str, Dict[int, Dict[str, int]]] = {
            "macs": {},
            "ips": {},
            "claims": {},
            "reply_claims": {},
    ...
    def last_host_maps(self) -> Dict[str, Dict[int, Dict[str, int]]]:
        ...
        return {
            ...
            "claims": {
                int(idx): {mac: {ip: int(cnt) for ip, cnt in ip_counts.items()} for mac, ip_counts in counts.items()}
                for idx, counts in self._last_host_maps.get("claims", {}).items()
            },
            "reply_claims": {
                int(idx): {mac: {ip: int(cnt) for ip, cnt in ip_counts.items()} for mac, ip_counts in counts.items()}
                for idx, counts in self._last_host_maps.get("reply_claims", {}).items()
            },
        }
    ...
    def load_host_maps(self, pcap_path: Path, directory: Path | None = None) -> Dict[str, Dict[int, Dict[str, int]]] | None:
        ...
        return {
            ...
            "claims": {
                ...
                for idx, counts in data.get("claims", {}).items()
            },
            "reply_claims": {
                int(idx): {
                    mac: {ip: int(count) for ip, count in mac_claims.items()}
                ...
                for idx, counts in data.get("reply_claims", {}).items()
            },
        }
    ...
    def _build_host_maps(self, windows: Iterable[Window]) -> Dict[str, Dict[int, Dict[str, int]]]:
        ...
        claims: Dict[int, Dict[str, Dict[str, int]]] = {}
        reply_claims: Dict[int, Dict[str, Dict[str, int]]] = {}
        for window in windows:
            mac_counts = Counter()
            ip_counts = Counter()
            claim_counts: Dict[str, Dict[str, int]] = {}
            reply_counts: Dict[str, Dict[str, int]] = {}
            for pkt in window.packets:
                if (pkt.protocol or "").lower() != "arp":
                    continue
                mac = pkt.arp_sender_mac or pkt.src_mac
                ip_addr = pkt.arp_sender_ip or pkt.src_ip
                if mac:
                    mac_counts[mac] += 1
                if ip_addr:
                    ip_counts[ip_addr] += 1
                if mac and ip_addr:
                    current = claim_counts.setdefault(mac, {})
                    current[ip_addr] = int(current.get(ip_addr, 0)) + 1
                    if (pkt.arp_opcode or 0) == 2:
                        reply_map = reply_counts.setdefault(mac, {})
                        reply_map[ip_addr] = int(reply_map.get(ip_addr, 0)) + 1
            if mac_counts:
            ...
            if claim_counts:
                claims[int(window.index)] = {
                    mac: {ip: int(count) for ip, count in claim_map.items()}
                    for mac, claim_map in claim_counts.items()
                }
            if reply_counts:
                reply_claims[int(window.index)] = {
                    mac: {ip: int(count) for ip, count in claim_map.items()}
